...
function sameReceipt(graph: ReceiptEvidence, rpc: ReceiptEvidence): boolean {
  return (
    HEX_FIELDS.every(
      (key) => String(graph[key]).toLowerCase() === String(rpc[key]).toLowerCase(),
...
export function classifyReceipt(input: {
  ...
  if (
    input.unavailable ||
    !input.graph ||
    input.rpcHead === undefined ||
    !input.expectedContract ||
    input.expectedChainId === undefined ||
    !Number.isInteger(input.maxLag) ||
    input.maxLag < 0
  ) {
    return {
      status: "UNAVAILABLE",
      reason: input.unavailable ?? "verification dependencies are not configured",
    };
  }
  ...
  const evidence: VerificationEvidence = {
    graph,
    rpcHead: input.rpcHead,
    ...(input.rpc ? { rpc: input.rpc } : {}),
  };
  if (
    !Number.isSafeInteger(graph.blockNumber) ||
    graph.blockNumber < 0 ||
    !Number.isSafeInteger(input.rpcHead) ||
    input.rpcHead < 0 ||
    graph.hasIndexingErrors ||
    graph.blockNumber > input.rpcHead
  ) {
  ...
  if (input.rpcHead - graph.blockNumber > input.maxLag) {
    return {
      status: "PENDING",
      reason: "Subgraph is behind the configured freshness threshold",
      evidence,
    };
  }
  ...
  if (!graph.receipt) {
    return {
      ...
      reason: "No receipt exists in the indexed state",
      evidence,
    };
  }
  ...
  if (!Number.isSafeInteger(receiptBlock) || receiptBlock < 0 || receiptBlock > graph.blockNumber) {
    return {
      status: "INVALID",
      reason: "Receipt block is inconsistent with Graph metadata",
      evidence,
    };
  }
  if (input.atBlock !== undefined && receiptBlock > input.atBlock) {
    return {
      status: "NOT_FOUND_AT_BLOCK",
      reason: "Receipt was created after the requested block",
      evidence,
    };
  }
  if (!input.rpc) {
  ...
  if (input.rpc.chainId !== input.expectedChainId) {
    return {
      status: "INVALID",
      reason: "RPC chain does not match the configured deployment",
      evidence,
    };
  }
  ...
  if (!sameReceipt(receipt, input.rpc.receipt)) {
    return {
      status: "INVALID",
      reason: "Graph receipt does not match the canonical RPC event",
      evidence,
    };
  }
  if (input.rpc.receipt.blockTimestamp !== input.rpc.receipt.settledAt) {
    return {
      status: "INVALID",
      reason: "Receipt settlement time does not match its block",
      evidence,
    };
  }
  ...
  return {
    ...
    reason: "Graph entity matches the canonical RPC event",
    evidence,
  };
}
