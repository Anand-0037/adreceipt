...
import {
  address,
  bytes32,
  configSchema as quoteConfigSchema,
  hashQuote,
  hashSubject,
  positiveUint,
  uint,
...
export const onCronTrigger = (runtime: TeeRuntime<Config>): string => {
  ...
  if (
    lower(config.ticket.publisher) !== lower(config.subject.publisher) ||
  ...
  if (
    lower(config.ticket.asset) !== lower(config.asset) ||
    BigInt(config.ticket.amount) !== BigInt(config.amount) ||
    BigInt(config.ticket.validUntil) !== BigInt(config.validUntil) ||
    BigInt(config.ticket.chainId) !== BigInt(config.chainId) ||
    lower(config.ticket.settlementContract) !== lower(config.settlementContract) ||
  ...
  const contextMatches =
    (["campaignId", "payer", "recipient", "asset", "chainId", "settlementContract"] as const).every(
      (key) => lower(policy[key]) === lower(config[key]),
  ...
  const topicOverlap = config.context.topics.filter((topic) =>
    policy.allowedTopics.includes(topic),
  ).length;
  // Mirror the public matcher's 0.72 relevance floor using integer math. A
  // context may contain extra topics; the campaign must still cover enough of
  // its own declared targeting before bid or payment can matter.
  const topicRelevant =
    70 * topicOverlap + 30 * policy.allowedTopics.length >= 72 * policy.allowedTopics.length;
  const intentAllowed = policy.allowedIntents.includes(config.context.intent);
  const contextSafe =
    config.context.ageEligibility === "ADULT_DECLARED" &&
    config.context.sensitiveClass === "NONE" &&
    config.context.personalization === false;
  const revisionBound =
    lower(policy.campaignRevisionHash) === lower(config.ticket.campaignRevisionHash);
