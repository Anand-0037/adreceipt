...
export const uint = (bits: number) =>
  z
    ...
    .refine(
      (value) =>
        /^(0|[1-9][0-9]*)$/.test(value) && value.length <= 78 && BigInt(value) < 1n << BigInt(bits),
      `Exceeds uint${bits}`,
    );
export const positiveUint = uint(256).refine(
  (value) => /^[1-9][0-9]*$/.test(value) && value.length <= 78,
  "Must be positive",
);
...
export const subjectSchema = z
  .object({
    ...
    contentHash: bytes32,
    disclosureVersion: z.union([z.literal(1), z.literal(2)]),
  })
  .strict();
...
export function hashSubject(subject: z.infer<typeof subjectSchema>) {
  return hashStruct({
    primaryType: "SubjectV1",
    types: subjectTypes,
    data: {
      publisher: subject.publisher as Hex,
      placementId: subject.placementId as Hex,
      productRefHash: subject.productRefHash as Hex,
      contentHash: subject.contentHash as Hex,
...
export function hashQuote(config: Config) {
  return hashTypedData({
    ...
    primaryType: "PlacementQuoteV1",
    types: quoteTypes,
    message: {
      schemaVersion: config.schemaVersion,
      campaignId: config.campaignId as Hex,
      subjectHash: config.subjectHash as Hex,
      payer: config.payer as Hex,
      recipient: config.recipient as Hex,
      asset: config.asset as Hex,
      amount: BigInt(config.amount),
      validUntil: BigInt(config.validUntil),
      nonce: config.nonce as Hex,
      chainId: BigInt(config.chainId),
