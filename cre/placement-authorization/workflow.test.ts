...
import {
	POLICY_PROOF_LEVEL, hashSanitizedContext, hashTicket, type PlacementTicket, type SanitizedContext,
} from './ticket'
...
const basePolicy = {
	...
	allowedTopics: ['developer-tools', 'cloud-hosting'],
	allowedIntents: ['compare', 'research'],
...
function scenario(overrides: {
	...
	const contextCommitment = hashSanitizedContext(baseContext, policy.contextSalt)
...
describe('confidential placement authorization', () => {
	...
	test('declines an intent outside the private allowlist', () => {
		const { result } = decide({ policy: { allowedIntents: ['purchase'] } })
		expect(result).toContain('eligible=false')
	})
	...
	test('declines a context the ticket did not commit to', () => {
		...
		const { result } = decide({ context: { ...baseContext, intent: 'research' } })
		expect(result).toContain('eligible=false')
	})
...
describe('structural ticket binding', () => {
	...
	for (const field of ['asset', 'amount', 'validUntil', 'campaignId'] as const) {
	...
	for (const key of ['subjectHash', 'quoteId', 'nonce', 'payer', 'recipient', 'chainId',
		'settlementContract'] as const) {
		test(`rejects a changed ${key} with an unchanged digest`, () => {
			const built = scenario()
			const current = String(built.config[key])
			const value = key === 'chainId'
				? '123'
				: current.replace(/.$/, current.endsWith('f') ? 'e' : 'f')
...
describe('confidentiality boundary', () => {
	test('does not leak private policy values into output', () => {
		const built = scenario({ policy: { maxBid: '123456789', contextSalt: `0x${'fe'.repeat(32)}` } })
	...
	test('keeps salts and allowlists out of the public report', () => {
		...
		expect(decoded).toContain(saltedPolicyCommitment(rawPolicy, basePolicy.commitmentSalt).slice(2))
	})
...
describe('cross-runtime hash vectors', () => {
	...
	test('derives the same subject and receipt id from the vector ticket', () => {
		...
		expect(hashQuote({ ...vectors.quote, chainId: String(vectors.quote.chainId) } as unknown as Config))
...
describe('workflow configuration', () => {
	...
	test('rejects an unknown context vocabulary', () => {
		...
		for (const context of [
			{ ...baseContext, topic: 'raw-query' },
			{ ...baseContext, intent: 'anything' },
			{ ...baseContext, surface: 'banner' },
			{ ...baseContext, locale: 'english' },
