...
var configSchema = objectType({
  schedule: stringType(),
  advertiser: stringType().regex(/^0x[0-9a-fA-F]{40}$/),
  domain: stringType().min(1),
  challenge: stringType().regex(/^0x[0-9a-fA-F]{64}$/),
  recordPrefix: stringType().default("_disclosed"),
  recordKey: stringType().default("disclosed-verification"),
  resolvers: arrayType(stringType().min(1)).min(2)
});
var REPORT_DOMAIN_VERIFICATION = 1;
var parseDohResponse = (body) => {
  let parsed;
  try {
  ...
  const records = (parsed.Answer ?? []).filter((a) => a.type === 16).map((a) => (a.data ?? "").replace(/"/g, "").trim()).filter((r) => r.length > 0);
...
var onCronTrigger = (runtime) => {
  ...
  const answers = config.resolvers.map((endpoint) => {
  ...
  const agreed = answered.length === 0 ? [] : answered[0].records.filter((record) => answered.every((a) => a.records.includes(record)));
  const conclusive = answered.length >= 2;
  const verified = conclusive && agreed.some((record) => record === expected);
  runtime.log(`domain check complete: resolvers_answered=${answered.length} conclusive=${conclusive} verified=${verified}`);
  if (!conclusive) {
    throw new Error("Inconclusive: fewer than two resolvers answered. Nothing reported.");
  }
  const donRuntime = runtime.usingTheDons();
  const payload = encodeAbiParameters(parseAbiParameters("address advertiser, bool verified, bytes32 challenge, uint64 checkedAt"), [config.advertiser, verified, config.challenge, 0n]);
