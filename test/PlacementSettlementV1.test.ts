...
describe("PlacementSettlementV1", function () {
  ...
  it("settles the exact payment and emits the frozen receipt", async function () {
    const { payer, publisher, recipient, token, settlement, subject, quote, sign, amount } =
    ...
    expect(
      await settlement.consumedNonces(await settlement.nonceKey(publisher.address, quote.nonce)),
    ).to.equal(true);
  });
  ...
  it("rejects the wrong payer", async function () {
    ...
    await expect(
      settlement.connect(outsider).settlePlacement(subject, quote, await sign()),
  ...
  it("rejects the wrong publisher signature", async function () {
    ...
    await expect(
      settlement.connect(payer).settlePlacement(subject, quote, signature),
  ...
  for (const field of [
    ...
    it(`rejects a changed ${field}`, async function () {
      ...
      if (field === "recipient" || field === "asset" || field === "settlementContract")
        changed[field] = f.outsider.address;
      else if (field === "amount") changed[field] += 1n;
      else if (field === "chainId" || field === "schemaVersion")
        changed[field] = BigInt(changed[field]) + 1n;
      else changed[field] = bytes32(`changed-${field}`);
      await expect(
        f.settlement.connect(f.payer).settlePlacement(f.subject, changed, await f.sign()),
      ).to.be.reverted;
    });
  }
  ...
  it("rejects an expired quote", async function () {
    ...
    await expect(
      f.settlement.connect(f.payer).settlePlacement(f.subject, f.quote, await f.sign()),
  ...
  it("rejects a consumed quote without emitting another receipt", async function () {
    ...
    await expect(
      f.settlement.connect(f.payer).settlePlacement(f.subject, f.quote, signature),
  ...
  it("rejects a reused publisher-scoped nonce but allows the same subject", async function () {
    ...
    await expect(
      f.settlement.connect(f.payer).settlePlacement(f.subject, second, await f.sign(second)),
    ).to.be.revertedWithCustomError(f.settlement, "NonceAlreadyConsumed");
    const third = { ...second, nonce: bytes32("nonce-2") };
    await expect(
      f.settlement.connect(f.payer).settlePlacement(f.subject, third, await f.sign(third)),
