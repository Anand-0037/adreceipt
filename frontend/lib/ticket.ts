...
export type ContextCommercialIntentBand = (typeof CONTEXT_COMMERCIAL_INTENT_BANDS)[number];
export type ContextSurface = (typeof CONTEXT_SURFACES)[number];
export type ContextAgeEligibility = (typeof CONTEXT_AGE_ELIGIBILITY)[number];
export type ContextSensitiveClass = (typeof CONTEXT_SENSITIVE_CLASSES)[number];
...
export interface SanitizedContextV2 {
  ...
  commercialIntentBand: ContextCommercialIntentBand;
  coarseLocale: string;
  surface: ContextSurface;
  ageEligibility: ContextAgeEligibility;
  sensitiveClass: ContextSensitiveClass;
...
export function parseSanitizedContext(value: unknown): SanitizedContextV2 {
  ...
  if (!Array.isArray(input.topics) || input.topics.length === 0) {
    throw new Error("Context topics must not be empty.");
  }
  const topics = [...new Set(input.topics.map(String))].sort() as ContextTopic[];
  if (
    topics.length > CONTEXT_TOPICS.length ||
    topics.some((topic) => !CONTEXT_TOPICS.includes(topic))
  ) {
  ...
  if (
    !CONTEXT_COMMERCIAL_INTENT_BANDS.includes(
      input.commercialIntentBand as ContextCommercialIntentBand,
    )
  ) {
    throw new Error("Unknown commercial intent band.");
  }
  if (!CONTEXT_AGE_ELIGIBILITY.includes(input.ageEligibility as ContextAgeEligibility)) {
    throw new Error("Unknown context age eligibility.");
  }
  if (!CONTEXT_SENSITIVE_CLASSES.includes(input.sensitiveClass as ContextSensitiveClass)) {
    throw new Error("Unknown sensitive context class.");
  }
  if (input.personalization !== false) throw new Error("Context personalization must be false.");
  if (typeof input.coarseLocale !== "string" || !LOCALE_PATTERN.test(input.coarseLocale)) {
  ...
  return {
    ...
    commercialIntentBand: input.commercialIntentBand as ContextCommercialIntentBand,
    coarseLocale: input.coarseLocale,
    surface: input.surface as ContextSurface,
    ageEligibility: input.ageEligibility as ContextAgeEligibility,
    sensitiveClass: input.sensitiveClass as ContextSensitiveClass,
    personalization: false,
    salt,
  };
}
...
export function hashSanitizedContext(context: SanitizedContextV2): string {
  ...
  const topicsHash = keccak256(abi.encode(["bytes32[]"], [parsed.topics.map(id)]));
  return TypedDataEncoder.hashStruct("SanitizedContextV2", SANITIZED_CONTEXT_V2_TYPES as never, {
    schemaVersion: parsed.schemaVersion,
    topicsHash,
    intentHash: id(parsed.intent),
    commercialIntentBand: CONTEXT_COMMERCIAL_INTENT_BANDS.indexOf(parsed.commercialIntentBand),
    coarseLocaleHash: id(parsed.coarseLocale),
    surfaceHash: id(parsed.surface),
...
export function parseTicket(value: unknown): PlacementTicketV2 {
  ...
  if (
    ...
    throw new Error("policyProofLevel must be 1 (CRE_SIMULATED) or 2 (CRE_ENFORCED).");
  }
  ...
  return {
    ...
    chainId: requireUint(input.chainId, 256, "Ticket chainId").toString(),
    settlementContract: requireAddress(input.settlementContract, "Ticket settlementContract"),
  };
}
