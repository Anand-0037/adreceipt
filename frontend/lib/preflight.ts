...
export function buildPreflight(input: {
  ...
  if (
    ...
    BigInt(ticket.chainId) !== BigInt(quote.chainId) ||
    !sameHex(ticket.settlementContract, quote.settlementContract) ||
    !sameHex(ticket.campaignId, quote.campaignId)
  ) {
    throw new Error(
      "The ticket and the quote disagree on asset, amount, expiry, chain, contract or campaign.",
    );
  }
